...
function compileAndStart(tscArgs, execCmd, execArgs, withColors) {
  ...
  util.println('Starting compilation...', true, withColors ? util.colors.cyan : null);
  ...
  spawn('node', [tscPath].concat(tscArgs)).stdout.on('data', function(data) {
    ...
    if (msg.indexOf('Watching for file changes') !== -1) {
      ...
      util.println((childProcess ? 'Restarting' : 'Starting') + ': ' + execStr + '...', true, withColors ? util.colors.green : null);
